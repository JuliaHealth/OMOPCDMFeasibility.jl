@testset "create_individual_profiles" begin
    ...
    @test_throws ArgumentError create_individual_profiles(
        cohort_df=test_cohort_df, conn=TEST_CONN, covariate_funcs=Function[], schema="main", dialect=:sqlite
    )
    @test_throws ArgumentError create_individual_profiles(
        conn=TEST_CONN, covariate_funcs=[occ.GetPatientGender], schema="main", dialect=:sqlite
    )
    @test_throws ArgumentError create_individual_profiles(
        cohort_definition_id=1, cohort_df=test_cohort_df, conn=TEST_CONN, covariate_funcs=[occ.GetPatientGender], schema="main", dialect=:sqlite
    )
...
@testset "create_cartesian_profiles" begin
    ...
    @test_throws ArgumentError create_cartesian_profiles(
        cohort_df=test_cohort_df, conn=TEST_CONN, covariate_funcs=[occ.GetPatientGender], schema="main", dialect=:sqlite
    )
    @test_throws ArgumentError create_cartesian_profiles(
        conn=TEST_CONN, covariate_funcs=[occ.GetPatientGender, occ.GetPatientRace], schema="main", dialect=:sqlite
    )
...
@testset "Edge Cases and Integration" begin
    ...
    individual_result = create_individual_profiles(
        cohort_df=test_cohort_df, conn=TEST_CONN, covariate_funcs=[occ.GetPatientGender, occ.GetPatientRace], schema="main", dialect=:sqlite
    )
    ...
    @test_throws ArgumentError create_individual_profiles(
        cohort_df=invalid_df, conn=TEST_CONN, covariate_funcs=[occ.GetPatientGender], schema="main", dialect=:sqlite
    )
    @test_throws ArgumentError create_cartesian_profiles(
